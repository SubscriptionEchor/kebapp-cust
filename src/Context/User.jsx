...
export const UserProvider = (props) => {
    ...
    const addCartItem = async (
        ...
        if (key) {
            const existingItemIndex = cartItems.findIndex(item => item.key === key);
            cartItems.splice(existingItemIndex, 1, { ...result, key })
        }
        else {
            cartItems.push({ ...result, key: v4() })
        }
    ...
    const deleteRestaurantFromCart = (id) => {
        let updatedCart = cart?.filter(cart => cart?.restaurantId !== id)
        setCart(updatedCart)
