    ...
    <>
      ...
      <div style={{zIndex:100}} className="fixed bottom-0  left-0 right-0 bg-white rounded-t-2xl  max-h-[90vh] flex flex-col animate-slide-up">
                  ...
                  <div key={addon._id} className="space-y-3">
                    ...
                    <div className="space-y-2">
                      {addon?.optionData?.map((option) => {
                        ...
                        const buttonClasses = `w-full flex items-center justify-between p-3 rounded-lg border transition-all duration-200 ${
                          isSelected
        ...
        <div className={`flex-shrink-0 bg-white border-t border-gray-100 p-4 flex gap-3 ${
           (relatedCartItems?.length && !selectedOptionSetList?.length && !showVariationList && !isDecrementing) || isDecrementing ? 'hidden' : ''
        }`}>
          <button
            onClick={handleBack}
            disabled={step === 1}
          ...
          <button
            ...
            className={`flex-1 py-3 rounded-lg text-[13px] font-medium transition-colors ${
              (selectedVariation && (step === 1 || isValid) || (editingVariation&&step==1))
